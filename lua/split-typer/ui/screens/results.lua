...
function M.show_results(ctx)
  ...
  local decay = state.timed_mode and ctx.errs.get_session_decay(state.key_events) or nil
  ...
  add(string.format("    Hesitations: %d", stats.hesitation_count))
  hl(17, #lines[#lines], stats.hesitation_count == 0 and "SplitTyperGood" or (stats.hesitation_count <= 3 and "SplitTyperOk" or "SplitTyperBad"))
  add(string.format("    Hesit/100:   %.1f", stats.hesitations_per_100_chars))
  hl(17, #lines[#lines], stats.hesitations_per_100_chars <= 1 and "SplitTyperGood" or (stats.hesitations_per_100_chars <= 3 and "SplitTyperOk" or "SplitTyperBad"))
  add(string.format("    Avg pause:   %d ms", stats.avg_hesitation_ms))
  hl(17, #lines[#lines], stats.hesitation_count == 0 and "SplitTyperGood" or (stats.avg_hesitation_ms <= 1600 and "SplitTyperOk" or "SplitTyperBad"))
  add(string.format("    Best streak: %d", stats.best_streak))
  ...
  if coaching_info then
    add("")
    add(coaching_info.phase_line)
    hl(17, #lines[#lines], coaching_info.phase_hl)
    add(coaching_info.recommendation_line)
    hl(17, #lines[#lines], coaching_info.recommendation_hl)
  end
